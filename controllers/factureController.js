...
class FactureController {
   ...
   async getStoreInfo(transaction = null) {
        try {
                        console.log('Infos magasin récupérées:');

            const storeInfo = await StoreInfo.findOne({
                transaction,
                order: [['createdAt', 'DESC']] // Au cas où il y aurait plusieurs enregistrements
            });
            console.log('Infos magasin récupérées:', storeInfo);
            if (!storeInfo) {
                // Valeurs par défaut si pas d'informations du magasin
                return {
                    nom: 'Jouets Paradise',
    ...
    async createFacture(req, res) {
        try {
            ...
            // Compléter les données de la facture avec les informations du magasin
...
// Ajoute cette méthode dans la classe FactureController
async createFactureFromCommande(req, res) {
    try {
        const { idCommande } = req.params;
                console.log('Commande trouvée:');

        // Récupère la commande avec toutes les infos nécessaires
...
console.log('Infos magasin:')
        // Récupère les infos magasin
        const storeInfo = await this.getStoreInfo();
        console.log('Infos magasin:', storeInfo);
        // Calcule les montants HT/TVA
...
console.log('Données de la facture:', factureData);
        ...
        res.status(500).json({ message: 'Erreur création facture', error: error.message });
    }
}
   ...
   async generatePDF(req, res) {
        ...
        try {
            ...
            // Create stream to temporary file
            ...
            setTimeout(() => {
                try {
                    ...
                    console.log('Erreur lors de la suppression du fichier temporaire:', err.message);
                }
            }, 5000);
