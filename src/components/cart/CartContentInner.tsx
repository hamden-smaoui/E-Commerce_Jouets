...
// Composant CartItemWithPromotion
const CartItemWithPromotion = ({ item, index, onIncrement, onDecrement, onQuantityChange, onRemove, actionLoading }: any) => {
  ...
  const imageUrl = item.produit.images && item.produit.images.length > 0
    ? `${API_URL}${item.produit.images.sort((a:any, b:any) => a.rang - b.rang)[0].url}`
...
// Composant CartItemTotalDisplay
...
// Composant principal CartContentInner
function CartContentInner() {
  ...
  const { getTotals, itemTotals, removeItemTotal } = useCartPromotionContext();
              ...
              <div className="sticky top-6 space-y-6">
                <div className="bg-white rounded-2xl shadow-xl border-2 border-pink-200 p-6">
                  ...
                  <Link href="/site/passerCmd" className="block mt-6">
                    <button className="w-full bg-pink-500 text-white py-4 rounded-xl hover:bg-pink-600 font-extrabold transition-all transform hover:scale-105 shadow">
...
export default CartContentInner;